...
export class WinTracker {
  ...
  private createSymbolIcon(symbolId: number): { icon: any; isSpine: boolean } {
    // Try to create a spine-based icon for standard symbols (0–22)
    try {
      if (symbolId >= 0 && symbolId <= 22) {
        // Map multiplier symbols to their shared spines
        let spineKey: string;
        if (symbolId >= 0 && symbolId <= 9) {
          spineKey = `symbol_${symbolId}_spine`;
        } else if (symbolId >= 10 && symbolId <= 16) {
          spineKey = 'symbol_10_spine';
        } else if (symbolId >= 17 && symbolId <= 20) {
          spineKey = 'symbol_11_spine';
        } else if (symbolId >= 21 && symbolId <= 22) {
          spineKey = 'symbol_12_spine';
        } else {
          return { icon: null, isSpine: false };
        }
        if (ensureSpineFactory(this.scene, 'WinTracker')) {
          const spineAtlasKey = `${spineKey}-atlas`;
          const go: any = (this.scene.add as any).spine?.(0, 0, spineKey, spineAtlasKey);
          if (go) {
