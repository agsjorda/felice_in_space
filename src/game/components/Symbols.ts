...
export class Symbols {
  ...
  /**
   * Calculate total win from spinData using multiple fallback sources
   * This method tries to get totalWin directly from spinData, then falls back to
   * calculating from freespin items, multiplierValue, slot paylines, and tumbles
   */
  private calculateTotalWinFromSpinData(): number {
    console.log(`[Symbols] cachedTotalWin calculateTotalWinFromSpinData:`, this.currentSpinData);
    let totalWin = 0;
    try {
  ...
  private scheduleCongratsDialogAfterAutoplay(): void {
    ...
    this.scene.time.delayedCall(graceMs, () => {
      ...
      // - Prefer the explicit flag when set (and no pending retrigger)
      ...
      try {
        if (gameStateManager.isBonusFinished && !this.hasPendingScatterRetrigger()) {
  ...
  private async showCongratsDialogAfterDelay(): Promise<void> {
    ...
    // Re-check pending retrigger after async waits (state may have changed)
    if (this.hasPendingScatterRetrigger()) {
      console.log('[Symbols] Pending scatter retrigger after waits - skipping congrats, bonus will continue');
      return;
    }
    ...
    // let WIN_DIALOG_CLOSED show congrats when the user (or auto-close) closes the win dialog.
    const gameScene = this.scene as any;
    const deferMs = 150;
