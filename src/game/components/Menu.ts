...
interface GameScene extends Scene {
    ...
    /** Base bet only (no amplify). Use for HelpScreen payouts. */
...
export class Menu {
    ...
    private initializeTabContentContainers(scene: GameScene): void {
        ...
        // Help / Rules content is delegated to HelpScreen (use base bet so payouts do not change when amplify is enabled)
    ...
    private switchTab(scene: GameScene, tabContainers: ButtonContainer[], activeIndex: number, tabConfigs: any[]): void {
        ...
        if (isDemo && tabKey === 'history') {
            // Don't switch to history tab in demo mode - switch to first available non-history tab
            const firstNonHistoryIndex = tabConfigs.findIndex((config: any) => config.icon !== 'history' && config.icon !== 'close');
