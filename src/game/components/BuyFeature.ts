...
export class BuyFeature {
	 ...
	 * Uses the scatter Spine (`symbol_0_spine`) with its win animation if available,
	...
	private createScatterSymbolAnimation(scene: Scene): void {
		...
		const createWithSpine = () => {
			try {
				...
				if (!cacheJson.has('symbol_0_spine')) {
					if (this.scatterRetryCount < this.SCATTER_MAX_RETRIES) {
						this.scatterRetryCount++;
						console.warn(
							`[BuyFeature] Spine json 'symbol_0_spine' not ready. Retrying (${this.scatterRetryCount}/${this.SCATTER_MAX_RETRIES})...`
						);
	...
	private createBetInput(scene: Scene): void {
		...
		const betFormatted = formatCurrencyNumber(this.getCurrentBet());
